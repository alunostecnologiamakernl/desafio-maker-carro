...
O nosso Robô maker foi montado numa plataforma de prototipagem maker, usamos a criatividade sem medir esforços para demonstrar muito mais que um robô andando e sim atitudes um pouco mais inteligente no quesito desvio de obstáculos. Como o cenário físico, hardware, podem sofrer alterações e responder um pouco diferente do nosso protótipo, os alunos do grupo de estudo  desafio Maker da cidade de Nova Lima, orientados pelo Professor Fernando Fernandes e com apoio e parceria da Secretaria de Educação de Nova Lima, resolveram inovar e tentar diminuir esses impactos, usando a lógica. Então vamos aos procedimentos:

Após realizar toda montagem conforme as instruções nesse tutorial, baixar a lib PWMservo, colocar na pasta libraries onde esta instalada a IDE do arduino, carregar o código do nosso projeto e enviar para a placa. Com a shield Robótica SigeControl  já conectada, vamos realizar o ajuste fino de velocidade para o sincronismo dos motores, o sincronismo inversamente proporcional da velocidade em relação ao tempo de deslocamento, os limites de leitura das distâncias, nesse caso diretamente proporcionais à velocidade, para ações na lógica digital.

Parece complicado, mas o complicado faz parte da nossa inovação, todos os cálculos e formas para esses ajustes, já estão integrados no firmware, assim a única coisa que você vai precisar fazer é garantir que as baterias estejam boas, ligar o robô Maker e soltar em um ambiente sem obstáculos a frente, observando se a roda boba está bem retinha para não atrapalhar no ajuste e observar se o robô esta andando à frente corretamente. Caso não esteja assim, observe qual o lado ele está virando, na placa SigeControl temos 02 potenciômetro nativos e ligados na A1 e A2 do arduino, gire esses potenciômetros observando se a velocidade do motor irá alterar, assim use o potenciômetro A1 para controlar a velocidade do motor da esquerda e o potenciômetro A2 para controlar a velocidade do motor da direita. Use a regra de deslocamento do veículo robô de 02 motores, caso esteja indo para a direita, aumente a velocidade do motor, atuando no potenciômetro A2, ou diminua a velocidade do motor do sentido oposto, A1.  Assim poderá realizar um ajuste fino de velocidade, alinhando o movimento para frente o melhor possível.

É inversamente proporcional  a relação da velocidade e o tempo, para acionamento das funções de direção do robô, e são automaticamente ajustadas, juntamente com os limites de distância de obstáculo para tomada de decisão. Afinal em relação a velocidade e distância, precisamos aplicar a lógica  diretamente proporcional, quanto maior a velocidade, maior deve ser os limitadores de distância para evitar colisões sem o tempo suficiente para parar. Então o sistema está preparado com alguns padrões limites que podem ser alterados no código caso tenha necessidade,  mas na maioria dos casos é somente ajustar a velocidade dos motores que as demais características se ajustam.
...
Outra inovação conforme explicado acima é nos limitadores de distância, onde temos duas referências, a mínima distância de um objeto sólido que necessita de retorno e a mínima distância de um objeto sólido que o sistema irá decidir no processo de análise de obstáculos.

Outra coisa que fizemos foi a criação do botão maker, um botão usando o LDR nativo da SigeControl com finalidade de habilitar ou não uma função especial de leitura de quina, quando deslocando à frente sem obstáculos. Como  podemos ter resultados diferentes em relação aos obstáculos e ambiente, o botão maker foi criado para habilitar e desabilitar a função Quina, conforme as formas e métodos de obstáculos usados nos testes.

Falando em obstáculos, o análise da  distância é calculado como a biologia de deslocamento de um Morcego, quer dizer, emite um som, aguarda o retorno do eco e sabendo a velocidade de propagação do som no meio, conseguimos calcular a distância de um obstáculo, sendo assim é recomendado o uso de obstáculos sólidos e que propague bem o eco,  evitando obstáculos de tecidos, porosos e revestimentos que abafam o eco.  Para os testes, use obstáculos formando ângulos de 90 graus para melhor compreensão à lógica do sistema embarcado.
